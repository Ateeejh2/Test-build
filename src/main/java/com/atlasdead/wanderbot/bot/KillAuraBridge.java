 ...
 * separate. WanderBot yields combat rotation as soon as Myau may own it and
 * only retakes rotation after an explicit OFF acknowledgement. Unknown state
 * therefore fails closed instead of allowing both controllers to write
 * yaw/pitch at the same time.</p>
 */
final class KillAuraBridge {
    ...
    /**
     * Requests a confirmed OFF state before WanderBot-owned navigation may
     * write rotation. This protects startup/restart when Myau happened to be
     * enabled before WanderBot starts.
     */
    void beginNavigationSync() {
        desiredActive = false;
        clearTarget();
        sendKillAuraCommand(false);
        state = State.REQUESTING_OFF;
        pendingTicks = 0;
        retries = 0;
        faultRetryTicks = 0;
    ...
    /** True only when Myau has explicitly acknowledged OFF. */
    boolean isNavigationRotationSafe() {
        return state == State.DISABLED && !desiredActive;
    }
    ...
    private void beginRequest(boolean enable) {
        ...
        retries = 0;
        chatCooldown = CHAT_COOLDOWN_TICKS;
        // Myau may own the camera until an explicit OFF acknowledgement arrives.
        propagateAimOwnership(true);
    }
    ...
    private void advancePendingState() {
        ...
        // Unknown external state: keep WanderBot combat rotation disabled.
        propagateAimOwnership(true);
    }
