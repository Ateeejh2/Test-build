...
public final class BotController {
    ...
    public void start() {
        ...
        // Do not let startup navigation write rotation until Myau confirms OFF.
    ...
    private void tickInternal() {
        ...
        if (!startupSequence.isComplete()) {
            ...
            // Startup navigation directly owns camera rotation. If Myau is still
            // enabling/disabling or its state is unknown, wait until OFF is
            // acknowledged rather than allowing two camera writers to overlap.
            if (!killAuraBridge.isNavigationRotationSafe()) {
                movement.release();
                state = BotState.PLANNING;
                return;
            }
    ...
    public Path getNavigationPath() {
        return startupSequence.isComplete() ? null : startupSequence.getPath();
    }
