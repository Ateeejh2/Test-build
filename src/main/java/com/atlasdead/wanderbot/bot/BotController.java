...
public class BotController {
    ...
    public void tick() {
        ...
        if (!combatActive) {
            searchPatrol.tick(player, targetForSearch == null, path == null ? null : path.current(), goal);
        }
        ...
        boolean finishedPath = path != null && path.isFinished();
        boolean patrolRequestsReplan = searchPatrol.shouldReplan(target == null, goal);
        boolean needsImmediatePlan = path == null || finishedPath;
        boolean wantsReplan = replanPending || patrolRequestsReplan;

        if (needsImmediatePlan) {
            if (replanCooldown == 0) {
                movement.release();
                path = null;
                goal = null;
                ...
                // No usable path exists yet; wait for the planning cooldown.
                movement.release();
                state = BotState.REPLANNING;
            }
            return;
        ...
        if (wantsReplan && replanCooldown == 0) {
            movement.release();
            searchPatrol.clearActiveGoal();
            replanPending = false;
            state = BotState.PLANNING;
        ...
        // A replan is pending, but the current path is still usable and the
        // cooldown has not expired. Keep following the current path instead
        // of deleting it and freezing the bot.
        ...
        if (!combatActive && validationCooldown == 0) {
            ...
            if (!validator.validate(mc.theWorld, path, 9)) {
                ...
                validateCacheMisses++;
                requestReplan();
                if (replanPending && replanCooldown == 0) {
                    movement.release();
                    path = null;
                    goal = null;
                    replanPending = false;
    ...
    private void requestReplan() {
        // Do not destroy a healthy path merely because a replan was requested.
        // Mark it pending and keep following the current route until the
        // cooldown expires; the next tick will perform the replacement.
        replanPending = true;
