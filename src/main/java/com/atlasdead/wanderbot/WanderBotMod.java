...
public final class WanderBotMod {
    ...
    private static final Pattern KILLAURA_STATE = Pattern.compile(
            "^\\[Myau\\].*\\bKillAura\\b.*\\b(ON|OFF|ENABLED|DISABLED|TRUE|FALSE)\\b.*$",
    ...
    public void onChat(ClientChatReceivedEvent event) {
        ...
        java.util.regex.Matcher killAuraState = KILLAURA_STATE.matcher(plain);
        if (killAuraState.matches()) {
            String value = killAuraState.group(1);
            boolean enabled = "ON".equalsIgnoreCase(value)
                    || "ENABLED".equalsIgnoreCase(value)
                    || "TRUE".equalsIgnoreCase(value);
            BOT.applyKillAuraStateFromMessage(enabled);
        }
    ...
    private static String normalizeChat(String formatted) {
        ...
        return TextSanitizer.collapseWhitespace(stripped);
    }
}
