...
public class CombatDecisionEngine {
    ...
    public Result evaluate(EntityPlayerSP self, EntityPlayer target) {
        ...
        if (!targets.isViable(self, TargetTracker.LOADED_PLAYER_SCAN_RANGE)) return Result.none("target-invalid");
        ...
        if (selfRatio <= 0.32D && localPressure >= crowdThreshold()) {
        ...
        if (streak != null && streak.isHighValueStreak() && localPressure >= crowdThreshold() && distance > 4.5D) {
