...
public final class CombatControlModel {
    ...
    public Decision evaluate(EntityPlayerSP self, EntityPlayer target,
        ...
        if (targets != null && !targets.isViable(self, TargetTracker.LOCAL_COMBAT_TRACK_RANGE)) {
        ...
        if (nearby >= crowdThreshold()) desired += 0.20D;
        ...
        EntityPlayer best = targets == null ? null : targets.findBest(mc.theWorld, self, zones);
        ...
        if (nearby >= crowdThreshold() && distance > desired + 0.25D) {
            return new Decision(Phase.RETREAT, true, false, false, desired + 0.25D,
                    lateralBias(tactical), "multi-opponent-pressure");
        }
        if (nearby >= Math.max(1, crowdThreshold() - 1) && distance > desired + 0.85D && tactical != null && tactical.targetRetreating) {
        ...
        double combatRange = configuredCombatRange();
        if (tooClose) {
            return new Decision(Phase.MAINTAIN, true, false, targetVisible && distance <= combatRange,
                    desired, lateralBias(tactical), "hold-melee-gap");
        }

        boolean allowAttack = targetVisible && distance <= combatRange;
    ...
    private static int crowdThreshold() {
        return Math.max(1, Math.min(8, WanderBotSettings.crowdThreshold));
    }
